<template>
  <div class="info-con">
    ...
    <div v-if="['liaocheng','foshanrenyi','fsxt','lyxrm','beihairenyi', 'whhk', '925' ,'gdtj', 'stmz','nfyksdyy'].includes(HOSPITAL_ID)">
...
export default {
  ...
  methods: {
    ...
    qrcode() {
      //非聊城不执行
      if(!['liaocheng','fsxt','lyxrm','beihairenyi', 'whhk', '925', 'stmz','nfyksdyy'].includes(this.HOSPITAL_ID )) return false;
      let titleObject = this.userName + " " + this.passWord;
      ['foshanrenyi','fsxt','lyxrm','925','beihairenyi', 'whhk', 'stmz','nfyksdyy'].includes(this.HOSPITAL_ID ) && (titleObject=this.getBase(JSON.stringify({user:this.userName,auth: this.passWord})));
