        ...
        <el-row type="flex" class="row-bg" justify="space-between">
          <el-row class="left-part" type="flex">
            <el-row
              ...
              align="middle"
            >
              <img src="../../common/images/logo_dgdl.png" />
            </el-row>
  ...
  methods: {
    async toUnLock() {
      ...
      if (this.$store.state.shiftRecords.enterTime) {
        ...
        const enterTime =
          +this.$store.state.shiftRecords.enterTime +
          60000 * this.$store.state.shiftRecords.setupTime;
      ...
      if (this.$store.state.shiftRecords.shiftRecordID) {
        // 解锁
        const res = await unLockShiftRecord(
          this.$store.state.shiftRecords.shiftRecordID
        );
        // 清空
        this.$store.commit("changeShiftRecordID", "");
        this.$store.commit("changeEnterTime", "");
    ...
    remoteMethod(query) {
      ...
      if (!query || this.deptOptionList.length == 0 || this.deptName == query) {
