    ...
    async load(refresh) {
      ...
      if (!date)
        return this.$router.push(
          "/inpatientReport/" + moment().format("YYYY-MM-DD")
        );
      //下面跳接口
      if (!this.code) return;
      // this.code = '031202'
      ...
      const resData = await apis.listInpatientReport(
        this.code,
        startDate,
        endData
      );
      ...
      const papers = [];
      let pageNumber =
        getDataList.length / pageSize < 1
          ? 1
          : Math.ceil(getDataList.length / pageSize);
