...
export default {
  ...
  methods: {
    open(item, obj) {
      ...
      if (obj) {
        this.resultMeasuresList = obj.measures;
        this.resultTargetList = obj.targets;
      } else {
        this.resultMeasuresList = [];
        this.resultTargetList = [];
      }
    },
    close() {
      this.show = false;
    ...
    save() {
      ...
      if (obj.measures.length + obj.targets.length == 0) {
