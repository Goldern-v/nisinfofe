...
import { ipEnvUrl } from '../../config/ip.config';
export default {
  props: {
    queryTem: Object,
  },
  data() {
    return {
      ...
      intranetUrl: `${ipEnvUrl}/temperature/#/`,
      printAllUrl:`${ipEnvUrl}/temperature/#/printAll`
    };
  },
  methods: {
    getUrl() {
      return 'http://192.168.1.195:9091'
    },
    onPrint() {
  ...
  computed: {
    ...
    isMobile() {
      return this.$store.state.temperature.isMobile || localStorage.getItem('isMobile') == "true";
    }
  },
  beforeDestroy() {
