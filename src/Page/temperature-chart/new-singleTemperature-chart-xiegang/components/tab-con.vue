    ...
    <div class="row-top">
      <div class="column-right">
        <el-date-picker
          ...
          type="date"
          size="small"
          style="width: 110px; margin-left: 2%"
          format="yyyy-MM-dd"
          placeholder="选择日期"
  ...
  methods: {
    ...
    async getList() {
      ...
      await getfieldList({
        ...
        visitId: this.patientInfo.visitId,
        wardCode: this.patientInfo.wardCode,
        recordDate: moment(new Date(this.query.entryDate)).format("YYYY-MM-DD") + "  " +this.query.entryTime,
      }).then((res) => {
      ...
      switch (trage) {
        ...
        case "线上体温":
          ...
          let x = {
            血压: {
              value: val,
              reg: ['qhwy'].includes(this.HOSPITAL_ID)?[60,140]:[90, 140],
              trage
            },
          };
          return x;
    ...
    async saveVitalSign(value) {
      ...
      obj.map((item) => {
        ...
        if(item.vitalValue !== "" &&this.checkItem.includes(item.vitalSigns)){
