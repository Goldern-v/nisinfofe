              ...
              :class="
                [
                  ...
                  item.recordDate.match(
                    `${formatDate(query.entryDate)}  ${formatTime(query.time)}`
                  )
                    ? 'active'
                    : '',
                ].join(' ')
...
export default {
  ...
  data() {
    ...
    function myTime() {
      if (myHours >= 0 && myHours <= 4) {
        return "02";
      }
      if (myHours > 4 && myHours <= 8) {
        return "06";
      }
      if (myHours > 8 && myHours <= 12) {
        return "10";
      }
      if (myHours > 12 && myHours <= 16) {
        return "14";
      }
      if (myHours > 16 && myHours <= 20) {
        return "18";
      }
      if (myHours > 20 && myHours <= 23) {
        return "22";
      }
    }
    return {
      ...
      query: {
        time: new Date(2016, 9, 12, myTime()),
  ...
  computed: {
    isBaby(){
      return this.patientInfo.patientId&&this.patientInfo.patientId.includes('+')
    }
  },
  watch: {
    // query: {
  ...
  methods: {
    ...
    async getList() {
      ...
      await getfieldList({
        ...
        recordDate:
          moment(new Date(this.query.entryDate)).format("YYYY-MM-DD") +
          "  " +
          (this.query.time ? moment(new Date(this.query.time)).format("HH:mm:ss") : this.query.entryTime + ":00:00"),
      }).then((res) => {
    ...
    changeQuery(value) {
      ...
      if (
        ...
        let hours = temp.slice(12, 14)
        let minute = temp.slice(15, 17)
        let second = temp.slice(18, 20)
        this.query.entryTime = hours;
        this.query.time = new Date(2016, 9, 10, hours, minute, second)
    ...
    getViSigs() {
      let data = {
        ...
        timeStr: this.query.time ? moment(new Date(this.query.time)).format("HH:mm:ss") : this.query.entryTime + ":00:00",
        wardCode: this.patientInfo.wardCode,
      };
    ...
    async getVitalList() {
      ...
      await getmultiDict(wardCode).then((res) => {
        ...
        let data = [];
        let obj = [];
        const babyType = ['01','aurigo','funicle','breast','milk','033','3'] //婴儿显示项目
      const withOutbabyType = ['aurigo','funicle','breast','milk'] //成年人不显示项目
      if(this.isBaby){
        res.data.data.map((list)=>{
          if(babyType.includes(list.vitalCode)) babyDic[list.vitalSign] = list.vitalCode
        ...
        res.data.data.map((item, index) => {
          ...
          switch (item.signType) {
            ...
            case "other":
              if(!["表顶注释","表底注释"].includes(item.vitalSign)){
            if(!withOutbabyType.includes(item.vitalCode)) otherDic[item.vitalSign] = item.vitalCode;
              }
              break;
            default:
              break;
