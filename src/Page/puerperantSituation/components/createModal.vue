...
export default {
  ...
  methods: {
    ...
    handleSave() {
      ...
      if (params.birthBloodVolume && isNaN(Number(params.birthBloodVolume)))
        errMsg = "产后2h出血量 必须为数字";
        ...
        res => {
          ...
          if (res[2].data.data) {
            ...
            if (re2.pageMap) {
              ...
              let format_tmpl_explain = moment(
                tmpl_explain,
                "YYYY年MM月DD日HH时mm分",
                true
              ).format("YYYY-MM-DD HH:mm:ss");

              if (format_tmpl_explain !== "Invalid date")
                tmpl_explain = format_tmpl_explain;

              //胎膜破裂
              let tmp_option = re2From[`${formCode}_tmpl_option`];
              if (tmp_option == "人工") {
                newParams.artiBrokenMembraneTime =
                  new Date(tmpl_explain) !== "Invalid Date" ? tmpl_explain : "";
              } else if (tmp_option == "自然") {
                newParams.natureBrokenMenbraneTime =
                  new Date(tmpl_explain) !== "Invalid Date" ? tmpl_explain : "";
              }
              ...
              let format_temcsj_explain = moment(
                temcsj_explain,
                "YYYY年MM月DD日HH时mm分",
                true
              ).format("YYYY-MM-DD HH:mm:ss");

              if (format_temcsj_explain !== "Invalid date")
                temcsj_explain = format_temcsj_explain;

              if (new Date(temcsj_explain) == "Invalid Date") {
                temcsj_explain = "";
              } else {
                temcsj_explain = moment(temcsj_explain).format(
                  "YYYY-MM-DD HH:mm:ss"
                );
              }
              ...
              let hy_qk_option = "/";
              if (hy_option == "切开")
                hy_qk_option = re2From[`${formCode}_hy_qk_option`] || "/";

              //2h出血总量
              let ssss_ydfm_explain =
                re2From[`${formCode}_ssss_ydfm_explain`] || "";
              ...
              //血气分析ph值 对应 产科分娩记录单附录字段
              let xsefz_explain = re2From[`${formCode}_xsefz_explain`] || "";
    ...
    handleNumberChange(val, name) {
      ...
      if (targetChange) {
        setTimeout(
          _ => (document.querySelector(`.${name}-input input`).value = newVal)
        );
      }
    }
  },
