<template>
  <div :class="['shift-work', { fullPage }]" :style="{ height }">
    ...
    <CreateShiftWorkModal ref="createModal" @confirm="onCreateModalConfirm" />
  </div>
</template>
...
import CreateShiftWorkModal from "./components/create-modal";
...
export default {
  ...
  methods: {
    ...
    async load() {
      ...
      if (deptCode) {
        const res2 = await apis.getWardDailyList({
          deptCode,
          startDate,
          endDate,
        });
    ...
    async onCreateModalConfirm({ date, code, done }) {
      ...
      let res;
      try {
        res = await apis.createWardDaily({
          deptCode: code,
          changeShiftDate: d,
        });
      } catch (error) {
