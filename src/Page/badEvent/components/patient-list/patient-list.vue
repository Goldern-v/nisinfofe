...
export default {
  ...
  methods: {
    selectPatient(item) {
      console.log("selectPatient", item, item.bedLabel);

      // let p = localStorage["patientInfo"+item.bedLabel]

      // if(p){
      //   window.wid.rawData.methods.fillForm(JSON.parse(p))
      // }else{
      //   window.wid.rawData.methods.fillForm({})
      // }
      localStorage["BadEvent-CurrentBedLabel"] = item.bedLabel;
      window.wid.rawData.methods.fillForm(item || {});
      window.wid.rawData.methods.fillDataToPost();
      // this.$store.commit("upPatientInfo", item);
      console.log("!!!params", this.$route.params);
      // this.$router.push({name: 'badEventEdit', params: {id:row.id, name:row.badEventName,type:row.eventType,operation:'edit'}})
      this.$router.replace(
        {
          name: "badEventEdit",
          params: this.$route.params,
          query: { bedLabel: item.bedLabel }
        }
        // () => {
    ...
    isActive(item) {
      return (
        item.bedLabel === this.$route.query.bedLabel ||
        item.bedLabel === localStorage["BadEvent-CurrentBedLabel"]
      );
    },
    toOpenLeft() {
