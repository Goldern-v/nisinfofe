  ...
  >
    <ElForm class="edit-modal-form" style="margin-bottom: 20px" label-width="100px">
      ...
      <ElFormItem label="巡视内容：" required>
        <ElInput type="textarea" :rows="2" v-model="form.content" />
      </ElFormItem>
    </ElForm>
    <div slot="button">
...
import {addNursingVisitPda} from "../../api/index";
  ...
  methods: {
    ...
    open(data,flag=false) {
      ...
      this.form = {
        ...
        operateDate: moment(moment(data.operateDate).toDate().getTime()).format("YYYY-MM-DD HH:mm:ss") || new Date(),//  --巡视日期
        operateDateNew: moment(moment(data.operateDate).toDate().getTime()).format("YYYY-MM-DD HH:mm:ss") || new Date(),//  --巡视新日期
        content:data.content, //--巡视内容
    ...
    async onConfirm() {
      if (this.form.content) {
        ...
        await addNursingVisitPda(this.form)
          .then(res => {
            this.loading = false;
            this.$message({
              type: "success",
