export default function(win) {
  ...
  root.setAttribute("style", "width: 1250px");
  ...
  while ((row = patients.shift())) {
    ...
    if (!page || page.offsetHeight + h > maxTableH) {
    ...
    if (!patients.length) {
      while (page.offsetHeight + h < maxTableH) {
  ...
  if (root.children.length > 0) {
    ...
    children.forEach((child, i, children) => {
      ...
      child.style = `position: relative; height: ${maxH}px;`;
