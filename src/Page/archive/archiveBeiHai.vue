...
import PDFDetailsModal from './modal/pdf-details-modal.vue';
export default {
  mixins: [common, mixin],
  components: {
    nullText,
  ...
  methods: {
    onPDFDetails(item) {
      this.$refs.pdfDetails.open(item);
    },
    onUpdateReload(value) {
      this.reloadPDFList = value;
    },
    allArchive(){
      let params = {
        pageSize:"",
    ...
    // 生成归档文件
    generateArchive(item, reloadPDFList) {
      this.$message({
        type: "success",
        message: "正在转pdf，请稍等",
      });
      generateArchiveNew(item.patientId, item.visitId).then((rep) => {
        this.reloadPDFList = !!reloadPDFList;
        this.getArchiveList();
      });
    },
    // 预览归档文件
