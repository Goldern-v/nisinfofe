<template>
  <div class="shift-work-detail" v-loading="loading">
      ...
      <div class="paper" v-else>
        <div ref="printable" data-print-style="height: auto;">
          ...
          <TableICU
            ...
            class="table"
            :fixedTh="fixedTh"
            data-print-style="height: auto;display: none;"
            :columns="columns"
            :editable="!allSigned"
            :record="record"
    ...
    <DateModal ref="dateModal"></DateModal>
  </div>
</template>
...
import formatter from "./print-formatter-icu";
import Button from "./components/button";
import TableICU from "./components/table-icu";
import PrintTableICU from "./components/print-table-icu";
...
export default {
  ...
  created() {
    this.bus.$on("printTable", (data) => {
      this.printTable(data.startDate, data.endDate);
    });
  },
  mounted() {
    if (this.deptCode) {
  ...
  methods: {
    ...
    async load() {
      ...
      try {
        const {
          data: { data },
        } = await apis.getShiftRecordICU(id);
        console.log(data);
        ...
        //     Array.from({ length: 1 - patients.length }).map(() => ({
        //       ...defaultPatient,
        //     }))
        //   );
        // }
    ...
    async onSave(tip) {
      await apis.updateShiftRecordICU({
        ...this.record,
    ...
    onSignModalOpen(type) {
      ...
      //     await apis.signShiftRecordICU(this.record.id, type, username, password);
    ...
    onDelSignModalOpen(type, sourceEmpNo) {
      ...
      //   title: "取消签名确认",
      //   callback: async ({ username, password }) => {
      //     await apis.delSignShiftRecordICU(
      //       this.record.id,
      //       username,
      //       password,
      //       type,
    ...
    async onRemove() {
      // this.$refs.signModal.open({
      //   callback: async ({ username, password }) => {
      //     await apis.removeShiftRecordICU(this.record.id, username, password);
    ...
    async printTable(startDate, endDate) {
      this.loading = true;
      const {
        data: { data },
      } = await apis.getMulShiftRecordICU(this.deptCode, startDate, endDate);
      this.patients = data;
      if (this.patients && !this.patients.length) {
        this.loading = false;
        return this.$message.warning("该时间段暂无交班志");
      }
      this.fixedTh = false;
      this.$nextTick(async () => {
