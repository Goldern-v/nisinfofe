  ...
  <div>
    <SweetModal ref="modal" :title="title" :modal-width="450">
      ...
      >
        ...
        <ElFormItem prop="object" label="教育对象：" v-if="HOSPITAL_ID=='beihairenyi'">
        ...
        <ElFormItem prop="object" label="教育对象：" v-else>
        ...
        <ElFormItem prop="method" label="教育方式：" v-if="HOSPITAL_ID=='beihairenyi'">
          <ElSelect v-model="form.method" multiple>
        ...
        <ElFormItem prop="method" label="教育方法：" v-else>
        ...
        <ElFormItem prop="assessment" label="教育评估：" v-if="HOSPITAL_ID=='beihairenyi'">
          <ElSelect v-model="form.assessment" multiple>
            <ElOption
        ...
        <ElFormItem prop="assessment" label="教育评估：" v-else>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        // object: [{ required: true, message: "请输入教育对象", trigger: "blur" }],
        // method: [
  ...
  methods: {
    ...
    setParams() {
      ...
      //教育对象
      let educationItem = educationObiect.filter(
        item => {
          console.log(this.form.object);
          if(this.form.object.indexOf(item.value) != -1){
           return item.text
          }
        }
      ); // 教育对象
      ...
      //教育方式
       let objectStr = educationItem.map(item=>{
        return item.text
      })
      let object = objectStr.toString()
      
      let itemMethod = educationMethod.filter(
        item => {
          // item.value === this.form.method
           if(this.form.method.indexOf(item.value) != -1){
           return item.text
          }
        }
      );
      let methodStr = itemMethod.map(item=>{
        return item.text
      })
      let haveValue = methodStr.toString()
      // let method = (haveValue.length > 0 && haveValue[0].text) || "在线"; // 教育方法
      let method = haveValue.length > 0 ? haveValue : "在线"; // 教育方法
      console.log(method);
      //教育方式

      // 教育评估
      
      let assessmentItem = educationAssessment.filter(
         item => {
          // item.value === this.form.method
           if(this.form.assessment.indexOf(item.value) != -1){
           return item.text
          }
        }
        // item => item.value === this.form.assessment
      ); 
       let assessmentStr = assessmentItem.map(item=>{
        return item.text
      })
      let assessment =  assessmentStr.toString()
