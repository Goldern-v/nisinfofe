  ...
  <div class="right-part-contain" :class="{ fullPageRecord,ifCloseOriginTem }">
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <patientInfo
            ...
            v-if="this.$route.query.patientId && (['lingcheng', 'lyxrm', 'xiegang', 'whhk', 'stmz', 'whsl', 'hzly'].includes(HOSPITAL_ID))"
          ></patientInfo>
        </div>
      </div>
     </template>
     <!-- 其他记录单或者血糖，健康 -->
      ...
      <RelationFormModal/>
      <!-- 电子病例弹窗 -->
      <doctorEmr v-if="['foshanrenyi','huadu','zhzxy','dglb','nfyksdyy', 'ytll','925'].includes(HOSPITAL_ID)" />
    </div>
  </div>
</template>
