...
export default {
  ...
  methods: {
    ...
    //     try {
    //       this.sheetInfo.sheetType = this.typeList.find((item) => {
    //         return item.deptCode == deptCode
    //       }).recordCode
    //       switch (this.sheetInfo.sheetType) {
    //         case 'endocrinologynew': getListData1(); break
    //         case 'gynecologynew': getListData2(); break
    //         case 'jointnew': getListData3(); break
    //         case 'neurology': getListData4(); break
    //       }
    //       this.getSheetData(true)
    //     }
    //     catch (error) {
    //       console.log(error)
    //       this.$message.warning('该科室没有开通护理记录单，请联系管理员')
    //     }
    //   })
    // },
    addNursingOrderSheetPage() {
