...
export default {
  ...
  methods: {
    getData(id) {
      detailNew(id)
        .then((res) => {
          ...
          this.consultation.consultationDeptCode =
            res.data.data.nursingConsultation.consultationDeptCode ?
            res.data.data.nursingConsultation.consultationDeptCode.split(',') : [];
          let isMember = false;
        ...
        nursingConsultation: {
          ...
          consultationDeptName:
             this.applyDepartmentOptions.filter(
              (v) => this.consultation.consultationDeptCode.includes(v.code)
            ).map(v => v.name).join(','),
          consultationDeptCode: this.consultation.consultationDeptCode.join(',')
        },
    ...
    submit() {
      ...
      if (
        data.nursingConsultation.deptCode
        && data.nursingConsultation.consultationDeptCode.includes(
          data.nursingConsultation.deptCode
        )
      ) {
