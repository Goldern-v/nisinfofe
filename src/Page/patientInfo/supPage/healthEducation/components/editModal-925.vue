  ...
  <div>
    <SweetModal ref="modal" :title="title" :modal-width="450">
      ...
      >
        ...
        <ElFormItem prop="date" label="教育时间：">
          <el-date-picker
  ...
  methods: {
    ...
    open(title, form) {
      ...
      if (form) {
        ...
        let method = []
         if(form["教育方法"] && form["教育方法"]!=''){
          method = educationMethod.reduce((total,item,index)=>{
            return total=form["教育方法"].split(",").includes(item.text)?total.concat(item.value):total
          },[]);
        }
      ...
      if (query !== "") {
        ...
        try {
          let params = {
            type: "",
            name: query,
            deptCode: this.$route.query.wardCode
          };
    ...
    setParams() {
      ...
      let haveValue =  educationMethod.reduce((total,item,index)=>{
        let findModel=this.form.method.find(findItem=>findItem==item.value);
        (findModel) && (total.push(item.text));
        return total
      },[]).join(",");
