<template>
  <div class="d-table">
    ...
    >
      ...
      <el-table-column prop="nextDate" label="下次巡视日期" width="150px" align="center">
        <template slot-scope="scope">
          <div>{{scope.row.nextDate | ymdhm}}</div>
        </template>
      </el-table-column>
      <el-table-column prop="num" label="巡视次数" min-width="80px" align="center">
        <template slot-scope="scope">
          <div>{{ (scope.row.num + '次') }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="operateDate" label="巡视时间" width="150px" align="center">
        <template slot-scope="scope">
...
export default {
  ...
  mounted() {
    console.log('isNursingRoundsAuthority', this.isNursingRoundsAuthority)
  },
  methods: {
    openViewModal(data) {
