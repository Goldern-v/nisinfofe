      ...
      <sweet-modal ref="modal" size="record" :title="title" :fullBtn="true" @close="beforeClose">
        <div class="record-modal-con" flex="">
          <div flex-box="3" style="width:0" class="record-box" v-loading="modalLoading">
            <iframe ref="iframeModal" class="iframe-form-con" v-if="url" :src="url" frameborder="0" @load="onload"></iframe>
          </div>
        ...
        <div slot="button" style="text-align:center">
          ...
          <el-button class="modal-btn" v-show="controlBtn.indexOf('signSave') > -1 || true" @click="saveForm" type="primary">{{saveButtonText}}</el-button>
        </div>
      </sweet-modal>
    </div>
    <!-- 签名确认 -->
  ...
  export default {
    ...
    methods: {
        ...
        if (this.url.indexOf("editMode") > -1) {
          ...
          evalDetail(
            ...
            if (this.formCode === 'form_pain_assessment') {
              formTableId = localStorage["firtPainFormID" + this.$route.query.patientId]||''
              ...
              if(formTableId){
                evalDetail(formTableId).then(resData => {
                  ...
                  res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_A"] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_A"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_A"]:pageMap["FORM_PAIN_ASSESSMENT_TTBW_A"] //疼痛部位
                  res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_B"] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_B"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_B"]:pageMap["FORM_PAIN_ASSESSMENT_TTBW_B"]
                  res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_C"] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_C"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_C"]:pageMap["FORM_PAIN_ASSESSMENT_TTBW_C"]
                  res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_D"] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_D"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_D"]:pageMap["FORM_PAIN_ASSESSMENT_TTBW_D"]
                  res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTFL"] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTFL"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTFL"]:pageMap["FORM_PAIN_ASSESSMENT_TTFL"] //疼痛分类
                  res.data.data.pageMap["FORM_PAIN_ASSESSMENT_RYSTTQK"] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_RYSTTQK"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_RYSTTQK"]:pageMap["FORM_PAIN_ASSESSMENT_RYSTTQK"] //入院时疼痛情况
                  ...
                  if(this.wid&&this.wid.setPainList){
                    let painList = {}
                    painList["FORM_PAIN_ASSESSMENT_TTBW_A".toLowerCase()] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_A"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_A"]:pageMap["FORM_PAIN_ASSESSMENT_TTBW_A"] //疼痛部位
                    painList["FORM_PAIN_ASSESSMENT_TTBW_B".toLowerCase()] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_B"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_B"]:pageMap["FORM_PAIN_ASSESSMENT_TTBW_B"]
                    painList["FORM_PAIN_ASSESSMENT_TTBW_C".toLowerCase()] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_C"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_C"]:pageMap["FORM_PAIN_ASSESSMENT_TTBW_C"]
                    painList["FORM_PAIN_ASSESSMENT_TTBW_D".toLowerCase()] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_D"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTBW_D"]:pageMap["FORM_PAIN_ASSESSMENT_TTBW_D"]
                    painList["FORM_PAIN_ASSESSMENT_TTFL".toLowerCase()] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTFL"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_TTFL"]:pageMap["FORM_PAIN_ASSESSMENT_TTFL"] //疼痛分类
                    painList["FORM_PAIN_ASSESSMENT_RYSTTQK".toLowerCase()] = res.data.data.pageMap["FORM_PAIN_ASSESSMENT_RYSTTQK"]?res.data.data.pageMap["FORM_PAIN_ASSESSMENT_RYSTTQK"]:pageMap["FORM_PAIN_ASSESSMENT_RYSTTQK"] //入院时疼痛情况
          ...
          // 加载第一张疼痛表 "form_pain_assessment"
            if (this.formCode === 'form_pain_assessment') {
              formTableId = localStorage["firtPainFormID" + this.$route.query.patientId]||''
                console.log("!!新建疼痛表!加载第一张疼痛表!!formTableId", formTableId);
              if(formTableId){
                evalDetail(formTableId).then(resData => {
                  console.log("!!新建疼痛表!加载第一张疼痛表!!onload_formField", resData);
                  let pageMap = resData.data.data.pageMap
                  let newpageMap = {}
                  let painList = {}
                  painList["FORM_PAIN_ASSESSMENT_TTBW_A".toLowerCase()] = pageMap["FORM_PAIN_ASSESSMENT_TTBW_A"] //疼痛部位
                  painList["FORM_PAIN_ASSESSMENT_TTBW_B".toLowerCase()] = pageMap["FORM_PAIN_ASSESSMENT_TTBW_B"]
                  painList["FORM_PAIN_ASSESSMENT_TTBW_C".toLowerCase()] = pageMap["FORM_PAIN_ASSESSMENT_TTBW_C"]
                  painList["FORM_PAIN_ASSESSMENT_TTBW_D".toLowerCase()] = pageMap["FORM_PAIN_ASSESSMENT_TTBW_D"]
                  painList["FORM_PAIN_ASSESSMENT_TTFL".toLowerCase()] = pageMap["FORM_PAIN_ASSESSMENT_TTFL"] //疼痛分类
                  painList["FORM_PAIN_ASSESSMENT_RYSTTQK".toLowerCase()] = pageMap["FORM_PAIN_ASSESSMENT_RYSTTQK"] //入院时疼痛情况
                  newpageMap['pageMap'] = painList

                  console.log("!!!加载第一张疼痛表!!painList", painList,this.formListData);
      ...
      saveForm() {
        ...
        // 如果要修改 启动签名
        // window.openModalName
        if (this.formStatus === '1' || this.formStatus === '2' || this.formCode === 'form_swallowing') {
          //  && this.saveButtonText === "签名"
          this.saveButtonText = "保存"
          // this.password = "";
          // this.$refs.signModal.open(this.saveFormbyPw);
        ...
        if (wid.validateForm) {
          ...
          var eval_score_input = wid.document.querySelector("input[name$='eval_score']");
      ...
      saveFormbyPw() {
        ...
        if (arguments.length == 2) {
          ...
          if (this.formStatus == '-1' || this.formStatus == '0' || this.formStatus == '1') {
            // postData['id'] = this.info.id;
            // postData['createSign'] = password;
            // 无审核人的表单，自动附带审核人签名密码
            if (this.formCode === 'form_fall' ||
        ...
        $(selectString, wid.document).each(function(index, array) {
          ...
          //
          if (self.name.indexOf("auditDate") > -1 && postData['auditDate'] != self.value) {
          ...
          if (self.type === "radio" || self.type === "date" || self.type === "time" || self.type === "number") {
        ...
        for (var item in allInputs) {
          if (allInputs.hasOwnProperty(item)) {
            if (allInputss.indexOf(allInputs[item].name) > -1 ||
              selectedInputs.indexOf(allInputs[item].name) > -1
            ) {
              continue;
            }
            if ((allInputs[item].type === "radio" || allInputs[item].type === "checkbox") && allInputs[item].result !== true) {
