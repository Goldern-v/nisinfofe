<template>
  <div class="modal-con">
    <sweet-modal ref="newRecord" size="big" title="创建新健康宣教" class="modal-record padding-0">
      <div class="title-bar" flex="cross:center">
        ...
        <el-input class="text-con" :placeholder="'搜索健康宣教'" icon="search" v-model="searchWord"></el-input>
      </div>
...
import { getEduFormTemplateByDeptCode } from "../api";
export default {
  mixins: [commonMixin],
  data() {
    return {
  ...
  methods: {
    ...
    getData() {
      ...
      getEduFormTemplateByDeptCode(this.this_deptCode).then(res => {
