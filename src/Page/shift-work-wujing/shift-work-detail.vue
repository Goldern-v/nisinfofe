      ...
      <div class="paper" v-else>
        <div ref="printable" data-print-style="height: auto;">
          ...
          >
            ...
            <tr class="normal-row">
              <td colspan="8" class="special-case-title" data-print-style="border-bottom: none;">
            ...
            },
            {
              ...
              render: row => {
                const status = row.patientStatus
                  ...
                  : "";
                return <div>{
                  [row.name + status, row.sex, row.age]
                    .filter(Boolean)
                    .map(v => <p>{v}</p>)
                  }
                  </div>;
          ...
          columns: [
            {
              ...
              render: row => (
                ...
                ? <button
                  onClick={() => this.onDelSignModalOpen('col',row.signatureEmpNo, row)}
                >{row.signature}</button>
                : row.id
                  ? <button onClick={() => this.onSignModalOpen('col', row)}>点击签名</button>
                  : ''
              )
            }
          ]
  ...
  methods: {
    ...
    onSignModalOpen(type, row = {}) {
      ...
      window.openSignModal(async (password, username) => {
        if (type === "col") {
          await apis.signShiftRecordByRow(
            row.id,
            username,
            password
          )
    ...
    onDelSignModalOpen(type, sourceEmpNo, row = {}) {
      // this.$refs.signModal.open({
      //   title: "取消签名确认",
      //   callback: async ({ username, password }) => {
      ...
      window.openSignModal(async (password, username) => {
        if (type === 'col') {
          await apis.delSignShiftRecordByRow(
            row.id,
            username,
            password,
            sourceEmpNo
