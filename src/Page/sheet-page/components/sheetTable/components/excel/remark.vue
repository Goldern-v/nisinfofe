...
注：1、神志：清醒√、嗜睡+、昏睡、浅昏迷++、深昏迷+++、模糊、谵妄、植物状态A、睁眼昏迷B        2、瞳孔观察—瞳孔对光反应：灵敏√  迟钝+  消失++
  ...
  <div class="bottomRemark">
    ...
    <div v-if="sheetInfo.sheetType == 'orthopaedic_sdlj'">
      书写说明：<br/>
      ⑴ 体位：A=抬高；B=抬高制动；C=外展中立位；D=外展位；<br/>
      ⑵ 颜色：A=红润；B=黑色；C=瘀紫色；D=暗红；E=苍白；F=花斑 G=粉红；
      ⑶ 皮温：A=温暖；B=低于健侧；C=稍低于健侧；D=冰凉；E=高于健侧；<br/>
      ⑷ 肿胀：A=无；B=I度；C=II度；D=III度；<br/>
      ⑸ 感觉：A=正常；B=稍麻；C=麻痹；D=消失；<br/>
      ⑹ 活动：A=正常；B=轻度受限；C=中度受限；D=重度受限；E=障碍；<br/>
      ⑺ 毛细血管反应：A=偏快＜1秒；B=正常1~2秒；C=偏慢＞2秒；D=消失；<br/>
      ⑻ 动脉搏动：A=正常；B=减弱；C=无；<br/>
      ⑼ 伤口敷料：A=干洁；B=少量渗血/渗液；C=中量渗血/渗液；<br/>
      ⑽ 石膏：A=过紧；B=过松；C=合适；<br/>
      ⑾ 牵引：A=过紧；B=过松；C=合适；<br/>
      ⑿ 指引功能锻炼：A=趾端伸屈；B=踝泵运动；C=股四头肌等长收缩运动；D=直腿抬高运动；E=膝关节屈曲运动；F=坐位训练；G=站立训练；H=扶拐行走训练；I=扶助；行架行走训练；J=未固定关节运动；K=指端伸屈；L=握拳训练；M=腕关节旋转运动；N=抬臂运动；O=肩关节环形运动；P=扩胸运动；<br/>
