...
注：1、神志：清醒√、嗜睡+、昏睡、浅昏迷++、深昏迷+++、模糊、谵妄、植物状态A、睁眼昏迷B        2、瞳孔观察—瞳孔对光反应：灵敏√  迟钝+  消失++
  ...
  <div class="bottomRemark">
    ...
    <div v-if="sheetInfo.sheetType == 'orthopaedic_sdlj'">
      书写说明：⑴ 肢体长度：健肢、患肢； ⑵ 体位：A=抬高；B=抬高制动；C=外展中立位；D=外展位；
      ...
      ⑼ 动脉搏动：A=正常；B=减弱；C=无；
      ⑽皮肤完整性：A=完整；B=发红；C=水泡；D=破损，渗液；
      ⑾护理措施：A=防足下垂；B=患肢保暖；C=受压处皮肤垫软垫;D=翻身、拍背，F=指导深呼吸及有效咳嗽等预防并发症； E=针眼护理；F=防压疮；
      ⑿ 伤口敷料：A=干洁；B=少量渗血/渗液；C=中量渗血/渗液；
      ⒀指引功能锻炼：A=踝泵运动；B=股四头肌等长收缩运动；C=直腿抬高运动；D=抬臂运动；E=膝关节屈曲运动；F=坐位训练；G=站立训练；H=扶拐行走训练；I=扶助；行架行走训练；J=未固定关节运动；
      ⒁ 石膏：A=过紧；B=过松；C=合适；
      ⒂ 牵引：A=过紧；B=过松；C=合适；
    </div>
     <!-- 顺德龙江 - 护理记录单（显微外科））-->
