  ...
  <div :class="['header-con',sheetInfo.sheetType === 'common_wj'?'wujing-big-title':'']">
    ...
    <div v-else>
      ...
      >
        ...
        <div
          class="bottom-line"
          style="min-width: 1080px;max-width: 1080px;font-size:12px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"
            >
          {{ processedDiagnosis[0] }}
        </div>
        <div
          class="bottom-line"
          style="margin-left:53px;min-width: 1080px;max-width: 1080px;font-size:12px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"
...
export default {
  ...
  computed: {
    ...
    processedDiagnosis(){
      ...
      let text = ''
      strArr.map(str=>{
       if(this.GetLength(text + str)>186){
          arr.push(text)
          text = str
        }else{
          text += str
