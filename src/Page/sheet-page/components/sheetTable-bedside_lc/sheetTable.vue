<template>
  <div>
    <div class="contant sheet-page-container" :style="data.titleModel.style" :class="{readOnly}">
      ...
      <div class="table-warpper" flex="cross:stretch" v-if="bedHeaderData.left.titleModel">
        <excel
          :data.sync="bedHeaderData.left"
          :index="index"
          :length="length"
          :scrollY="scrollY"
          :hasFiexHeader="true"
        ...
        ></excel>
        <div
          style="width: 10px;border-top: 1px solid #000;border-bottom: 1px solid #000;margin-top: 15px;margin-bottom: 15px;"
        ></div>
        <excel
          :data.sync="bedHeaderData.right"
          :index="index"
          :length="length"
          :scrollY="scrollY"
          :hasFiexHeader="true"
...
export default {
  ...
  computed: {
    ...
    bedHeaderData() {
      let bedHeaderData = {};
      bedHeaderData.left = {
        titleModel: this.data.titleModel,
        bodyModel: this.data.bodyModel.slice(0, 17)
      };
      bedHeaderData.right = {
        titleModel: this.data.titleModel,
        bodyModel: this.data.bodyModel.slice(17)
      };
      console.log(bedHeaderData);

      return bedHeaderData;
    }
  },
  created() {},
  update() {},
