<template>
  <div>
    ...
    <transition name="el-zoom-in-left" v-show="show">
      <div class="slide-con" >
        ...
        <div>
          ...
          <div class="search-con" v-if="['foshanrenyi'].includes(HOSPITAL_ID)&&templateType=='dept'">
...
export default {
  ...
  computed: {
    ...
    canDelete(){
      let flag=false
      if(this.selectedType==="特殊符号"||this.selectedType==="全部"||this.selectedType===""){
        flag=true
      }
      return flag
    },
  ...
  watch: {
    selectedType(val) {
      if (["特殊符号"].includes(val)) {
        return;
      }
      const wardCode = this.templateType === "dept" ? localStorage.wardCode : ""
      list(this.selectedType=='全部'?'':this.selectedType, wardCode, this.templateCode).then(res => {
        this.listMap = res.data.data.list;
      });
    },
    templateType(){
