<template>
  <div class="nursing-rules">
    ...
    <div class="main-contain">
      <div class="table-contain">
        <el-table :data="data" :height="tableHeight" border v-loading="pageLoadng" stripe>
          ...
          <el-table-column prop="name" label="制度名称" align="center" class-name="rule-name">
            <template slot-scope="scope">
    ...
    <sweet-modal ref="preview-modal" class="nursing-rules-preview-modal" :title="preview.title">
      ...
      <div v-else style="height: 300px;text-align:center;line-height:300px;">该文件格式不支持预览</div>
    </sweet-modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    downloadFile(scope) {
      getFileContent({ id: scope.row.id }).then(
        res => {
