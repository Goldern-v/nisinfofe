<template>
  <div class="contain"
    ...
    <div class="body-con"
      ...
      <div class="left-part">
        <patientList :data="data.bedList"
                     v-loading="patientListLoading"></patientList>
      </div>
...
import sheetModel, { addNursingOrderSheetPage, delNursingOrderSheetPage, initNursingOrderSheetPage, cleanData } from './sheet.js'
import {
  patients,
} from '@/api/lesion'
...
export default {
  ...
  addNursingOrderSheetPage() {
    ...
    breforeQuit(next) {
      if (!sheetInfo.isSave) {
        window.app.$confirm('评估单还未保存，离开将会丢失数据', '提示', {
    ...
    getHomePage(isFirst) {
      getHomePage(this.patientInfo.patientId, this.patientInfo.visitId).then(res => {
        ...
        this.sheetInfo.sheetMaxPage = (res.data.data && res.data.data.maxIndexNo) || 1
    ...
    getList(){
      // list
      orderApi.list(this.patientInfo.patientId, this.patientInfo.visitId).then(res=>{
        console.log("获取字典",res)
      })
    }
  },
  created() {
    ...
    this.bus.$on('toNursingOrderSheetPrintPage', (newWid) => {
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        let routeData = this.$router.resolve({ path: `/print/sheetNursingOrder`})
  ...
  components: {
    sheetTool, patientList, sheetTable, delPageModal, HjModal, signModal,
    specialModal,
