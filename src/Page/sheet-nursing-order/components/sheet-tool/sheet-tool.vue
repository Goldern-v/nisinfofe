<template>
  <div>
    ...
    <div class="tool-contain" flex="cross:center">
      ...
      <div class="item-box" flex="cross:center main:center" @click="emit('saveNursingOrderSheetPage')">
        <div class="text-con" flex="cross:center">
          保存
        </div>
      </div>
      ...
      <div class="item-box" flex="cross:center main:center" @click="bus.$emit('createNursingOrderPage',true)">
        <div class="text-con">
          新建护嘱单
        </div>
      </div>
      ...
      <el-select v-model="sheetInfo.selectBlock" @change="changeSelectBlock" value-key="id" placeholder="请选择护嘱单" class="select-con" popper-class="sheetSelect-con">
        <div class="head-con" flex="cross:stretch">
          <div class="col-1">护嘱单标题</div>
          <div class="col-2">科室</div>
          <div class="col-3">创建时间</div>
        </div>
        <el-option v-for="item in sheetBlockList" :key="item.id" :label="blockLabel(item)" :value="item">
          <div class="list-con" flex="cross:stretch">
            <!-- <div class="col-1" :title="item.recordName">{{item.recordName}}</div> -->
            <div class="col-1" :title="item.formTitle">{{item.formTitle||"护嘱单"}}</div>
            <div class="col-2" :title="item.deptName">{{item.deptName}}</div>
            <div class="col-3" :title="item.createDate">{{item.createDate}}</div>
          </div>
        </el-option>
      </el-select>
      ...
      <div class="item-box" flex="cross:center main:center" @click.stop="showSheetDescription">
        <div class="text-con">
          护嘱单说明
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      getNOBlockList(isAutoSelectLast=true) {
        if (this.patientInfo.patientId && this.patientInfo.visitId && this.deptCode) {
          // todo here...
          blockList(this.patientInfo.patientId, this.patientInfo.visitId, this.deptCode).then(res => {
            // console.log('blockList',res);
            this.sheetBlockList = res.data.data.list
            if(this.sheetBlockList.length>0){
              this.sheetBlockList = this.sheetBlockList.sort((a,b)=>{return a.id - b.id})
            }
            ...
            console.log('获取护嘱单列表',index,res,this.sheetBlockList,this.sheetInfo,this.patientInfo);
            ...
            console.log('获取护嘱单列表Sorted',res,this.sheetBlockList,this.sheetInfo,this.patientInfo);
          })
        }
      },
      addSheet() {
      ...
      createSheet(openForm=false, formTitle=this.sheetInfo.formTitle) {
        ...
        this.sheetInfo.orderContentCode = sheetInfo.orderContentCode||'orders:public:通用护嘱内容'
      ...
      delSheet() {
        if (!this.sheetInfo.selectBlock.id) return this.$message.warning('还没有选择护嘱单')
        this.$confirm('此操作将永久删除该护嘱单, 是否继续?', '提示', {
          ...
          window.openSignModal((password, empNo) => {
            blockDelete({id:this.sheetInfo.selectBlock.id, password:password, empNo:empNo}).then(res => {
      ...
      showSheetDescription(){
        ...
        this.$msgbox({
          ...
          message: h('p', {style: 'font-size:13px'}, [
            ...
            h('br'),
            h('p', null, '2、神经内科的护嘱主要是针对病危病重和一级护理及重度依赖的患者，由护理组长或护士长开出，由责任护士或护理班执行。'),
            h('br'),
            h('p', null, '3、本单挂在病人房间的宣传框内，每天交接班时组长或护士长当场进行开出或停止护嘱，并同时监控之前开出的护嘱有无执行。'),
            h('br'),
            h('p', null, '4、本护嘱单等到完全不需要护嘱或病人出院时责任护士将其收回，夹在护士站护嘱文件夹内存档。'),
            h('br'),
            h('p', null, '5、为方便大家工作，科室把常见的护嘱内容做成电子版，频次和协助或指导由开护嘱者自己写，如果有特殊的护嘱，请另外手写在上面。'),
          ]),
          callback: action => {}
        })
      }
    ...
    created() {
      ...
      this.bus.$on('createNursingOrderPage', (isOpenForm, formTitle=this.sheetInfo.formTitle) => {
      ...
      document.onkeydown = (e) => {
        ...
        if(currKeyStr == 'S' && (e.ctrlKey||e.metaKey)){//Ctrl+s
          e.preventDefault();
          e.returnvalue = false;
    ...
    watch: {
      pageArea() {
        ...
        if (startPage && endPage) {
          if (Number(endPage) - Number(startPage) >= 0 && Number(endPage) - Number(startPage) <= 20) {
