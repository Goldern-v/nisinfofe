<template>
  <div>
    <sweet-modal ref="modal" :modalWidth="460" :title="modalTitle" :enable-mobile-fullscreen="false">
      <div flex="main:justify cross:center" style="margin-bottom: 20px">
        <p for="" class="title" style="margin-right: 10px">类别：</p>
        <el-autocomplete style="width: 100%" v-model="groupName" :fetch-suggestions="querySearch" placeholder="请输入内容"></el-autocomplete>
      </div>
      <div flex="main:justify cross:center" style="margin-bottom: 20px">
...
<script>
  import {
    ...
    saveOrUpdate,
    list
  } from '@/Page/sheet-nursing-order/api/recordDesc.js'
  ...
  export default {
    ...
    methods: {
      ...
      post() {
        saveOrUpdate(this.groupName, this.title, this.content, this.id, this.formCode).then(res => {
          if (this.id) {
