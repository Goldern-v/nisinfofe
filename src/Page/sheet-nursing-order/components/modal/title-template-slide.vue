        ...
        <div>
          ...
          <div class="list-con">
            <div v-for="(item, key) in filterData" :key="key">
              <templateItem :formCode="formCode" :data="item" :key="item.id"></templateItem>
            </div>
          </div>
...
import templateItem from "./components/title-template-item.vue";
import addTemplateModal from "./add-title-template-modal.vue";
import bus from "vue-happy-bus";
import { listItem } from "@/Page/sheet-nursing-order/api/recordDesc.js";
...
export default {
  ...
  methods: {
    open(formCode='') {
      ...
      console.log('open:title-template-slide',this.formCode)
      this.getData();
      this.show = true;
    ...
    async getData() {
      let {
        data: { data }
      } = await listItem("自定义标题", this.formCode);
      this.listMap = data;
    },
    openAddModal() {
      this.$refs.addTemplateModal.open({'formCode':this.formCode});
    },
