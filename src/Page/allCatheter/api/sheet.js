...
export const saveBody = (patientId, visitId, data) => {
  ...
  return axios.post(
    `${apiPath}dvc/${sheetInfo.sheetType}/saveAll`,
    data
  );
};
export const saveRelObj = (formId, relObjs) => {
  ...
  return axios.post(`${apiPath}dvc/${sheetInfo.sheetType}/saveRelObj`, data);
};
export const showBody = (patientId, visitId) => {
  ...
  data.id = sheetInfo.selectBlock.id;
  data.multiRel = true;
  return axios.post(`${apiPath}dvc/${sheetInfo.sheetType}/list`, data);
};
export const saveTitle = data => {
  data.blockId = sheetInfo.selectBlock.id;
  return axios.post(`${apiPath}dvc/setting/save`, qs.stringify(data));
};
// 护理记录单标题
export const showTitle = (patientId, visitId) => {
  return axios.get(`${apiPath}dvc/setting/list/${sheetInfo.selectBlock.id}`);
};
// 护理记录单签名
export const sign = (patientId, visitId, data) => {
  ...
  data.visitId = visitId;
  data.id = sheetInfo.selectBlock.id;
  return axios.post(`${apiPath}dvc/${sheetInfo.sheetType}/signAll`, data);
};
// 护理记录单签名
export const cancelSign = data => {
  return axios.post(
    `${apiPath}dvc/${sheetInfo.sheetType}/cancelSign`,
    qs.stringify(data)
  );
};
// 删除页数
...
export const delRow = (id, password, empNo) => {
  return axios.post(
    `${apiPath}dvc/${sheetInfo.sheetType}/delete`,
...
export const markSave = data => {
  ...
  return axios.post(
    `${apiPath}dvc/${sheetInfo.sheetType}/sign/save`,
    qs.stringify(data)
  );
};
// 删除标记
export const markDelete = data => {
  data.recordCode = sheetInfo.sheetType;
  return axios.post(
    `${apiPath}dvc/${sheetInfo.sheetType}/sign/delete`,
    qs.stringify(data)
  );
};
// 获取标记集合
export const markList = (patientId, visitId) => {
  return axios.get(
    `${apiPath}dvc/${sheetInfo.sheetType}/sign/list/${sheetInfo.selectBlock.id}`
  );
};
