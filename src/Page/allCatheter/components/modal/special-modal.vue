    ...
    >
      <div id="specialForm">
        <div flex="cross:center" class="special-date-con">
          ...
          <input type="text" :disabled="recordDate != ''" v-model="staticObj.recordHour" @keyup="timeKey($event, staticObj, 'recordHour')">-->
        </div>
          ...
          <!-- <el-tab-pane label="自定义项目" name="2">
            <div class="custom-cell" v-for="(item, index) in customTitle" :key="item.key">
              <div class="custom-box" flex="cross:center">
                <div class="label" v-html="item.name"></div>
    ...
    post() {
      ...
      for (let i = 0; i < allDoc.length; i++) {
        ...
        if (
          ...
          if (GetLength(text) > (sheetInfo.sheetType == "picc" ? 29 : 23)) {
            result.push(text);
            text = allDoc[i];
          } else {
            text += allDoc[i];
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        if (this.record[i]) {
          ...
          console.log(sheetModel, this.lastZm, "qqqq");
