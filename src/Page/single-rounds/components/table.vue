<template>
  <div>
    <table class="table fixed-th" v-if="fixedTh">
      ...
      <thead>
      <tr>
        <th v-for="col of columns" :key="col.label" :colspan="getColSpan(col)" :rowspan="col.rowspan">{{col.label}}</th>
      </tr>
      <tr v-if="isMultiCol">
        <th v-for="col of realColumns" v-if="col.label" :key="col.label">{{col.label}}</th>
      </tr>
      </thead>
    </table>
...
<script>
  export default {
    ...
    methods: {
      ...
      onInputClick(rowIndex, prop, row,col) {
        if (!row.name) return
        if (!col.editable) return
        const value = this.data[rowIndex][prop] === '√' ? '×' : '√'
        this.$set(this.data[rowIndex], prop, value)
      },
