<template>
  ...
  >
    <div class="container">
      <div class="group">
        <div class="title">当前患者：</div>
        <div class="items" v-if="patients">
          <div class="item" v-for="(item,index) in patients" :key="item.patientId">
            <ElCheckbox
                :disabled="selectedKeys.includes(item.patientId)"
                :value="selectedKeys.includes(item.patientId) || isChecked(item)"
    ...
    <div class="footer" slot="button">
      <div class="left">
        <ElCheckbox :disabled="!patients" :value="patients.length === selectedItems.length" @change="onCheckAll">全选
        </ElCheckbox>
  ...
  export default {
    ...
    methods: {
      ...
      async loadPatients(deptCode, date, id) {
        ...
        // this.patients = data.data.filter(item => {
        //   return item.name
        // }).map(item => {
        //   return {
        //     bedNo: item.bedNo,
