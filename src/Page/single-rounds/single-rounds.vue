    ...
    <div class="left-side">
      ...
      <div class="list" v-loading="loading">
        <router-link
            ...
            :to="{name: 'singleRound', params: {code: record.deptCode, id: record.id}}"
            :key="record.id"
        >
          <div class="text">{{record.roundsDate}}</div>
  ...
  export default {
    ...
    watch: {
      deptCode(value, oldValue) {
        this.$router.push({path: '/singleRounds'})
      },
      '$route.params.code'() {
        this.load()
      }
    ...
    methods: {
      ...
      async onCreateModalConfirm({date, code, copy, done}) {
        ...
        this.$router.push({name: 'singleRound', params: {code, id: res.data.data.id}})
