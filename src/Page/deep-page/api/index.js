...
export const blockList = (patientId, visitId, wardCode) => {
  return axios.post(`${apiPath}formType/dvc/list`, {
    patientId,
    visitId,
    wardCode
  });
...
export const blockSave = (patientId, visitId, deptCode, code, type) => {
  return axios.post(`${apiPath}formType/dvc/save`, {
    patientId,
    visitId,
    deptCode,
    code,
...
export const blockDelete = id => {
  return axios.get(`${apiPath}formType/dvc/delete/${id}`);
};
...
export const multiDictInfo = (list, sheetType = "") => {
  ...
  return axios.post(
    `${apiPath}dept/multiDictInfo?code=${sheetType}&type=dvc&wardCode=${wardCode}`,
    list
  );
};
...
export const updateBlock = data => {
  return axios.post(
    `${apiPath}dvc/picc/update `,
    qs.stringify(data)
  );
};
// 记录列表
export const listRecord = deptCode =>
  axios.get(`${apiPath}/form/common/templates/dvc?deptCode=${deptCode}
  `);
