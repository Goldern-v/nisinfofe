<template>
  <div>
    <div
      class="contant deep-page-container"
    ...
    >
      ...
      <div v-if="sheetInfo.sheetType == 'picc'" style="font-size:12px;">
        <span>诊断：</span>
        <input
          type="text"
          class="bottom-line"
          style="width: 400px;outline: none; border: 0;border-bottom: 1px solid #000000;"
          v-model="patientInfo.diagnosis"
          :data-value="patientInfo.diagnosis"
          @click="openSetDiagsModal(patientInfo)"
        />
      </div>
      <infoTable
        v-if="sheetInfo.sheetType == 'picc'"
        :pageNo="index"
      ></infoTable>
      <infoTableIcupicc
        v-if="sheetInfo.sheetType == 'icupicc'"
        :pageNo="index"
      ></infoTableIcupicc>
...
import infoTableIcupicc from "./components/infoTable/infoTable-icupicc";
export default {
  props: {
    data: Object,
    index: Number,
