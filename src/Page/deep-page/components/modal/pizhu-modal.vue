  ...
  <div>
    <sweet-modal ref="modal"
      ...
      <div flex="cross:center">
        ...
        <el-select v-model="todoPerson" filterable placeholder="请选择" size="small" style="width: 120px">
          <el-option
      ...
      <div>
        <span style="color: #333">创建人：({{empname}})</span>
...
export default {
  ...
  methods: {
    init() {
      ...
      this.zkValue = ''
      this.password = ''
      this.empname = this.userInfo.empName
    ...
    open(tr, td) {
      ...
      this.init().then(res => {
        ...
        if(this.todoPersonList.find(item => code == signerNo)) {
    ...
    post() {
      ...
      let creatorName = this.empname
      let handlerNo = this.todoPerson
      if(!handlerNo) return this.$message.warning('需要选择批注处理人')
      let handlerName = this.todoPersonList.find(item => item.code == handlerNo).name

        saveMark(recordId, fieldEn, signType, content, qualityIndex, creatorNo, creatorName, handlerNo, handlerName).then(res => {
          if(fieldEn == 'all') {
            // 行标注
            this.tr.find(item => {
